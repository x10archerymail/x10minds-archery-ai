...
const HistoryView: React.FC<HistoryViewProps> = ({
  ...
  const accentClasses: Record<string, string> = {
    orange: "text-orange-500 border-orange-500 bg-orange-600",
    blue: "text-blue-500 border-blue-500 bg-blue-600",
    green: "text-green-500 border-green-500 bg-green-600",
    purple: "text-purple-500 border-purple-500 bg-purple-600",
  };

  const currentAccent = accentClasses[accentColor] || accentClasses.orange;
  const accentText = currentAccent.split(" border-")[0];
  const accentBorder = currentAccent.split(" border-")[1].split(" bg-")[0];
  const accentBg = currentAccent.split(" bg-")[1];
  ...
  const filteredSessions = sessions.filter((s) => {
    ...
    const matchesTab = activeTab === "all" || (s.type || "chat") === activeTab;
        ...
        <header className="mb-8">
          ...
          <div className="relative">
            ...
            <input
              ...
              } ${inputBorder} focus:border-${accentBorder}`}
            />
          </div>
          ...
          {sortedSessions.length === 0 ? (
            ...
            sortedSessions.map((session) => (
              <div
                ...
                className={`p-4 rounded-xl cursor-pointer transition-all group border relative ${cardClass} hover:border-${accentBorder}/50 hover:${
                  isDark ? "bg-neutral-800" : "bg-gray-50"
                } ${session.isPinned ? `border-${accentBorder}/30` : ""}`}
              >
                      ...
                      {renamingId === session.id ? (
                        ...
                        >
                          <input
                            ...
                            className={`px-2 py-1 text-sm font-bold border rounded-lg focus:outline-none focus:border-${accentBorder} ${inputBg} ${headerText} ${inputBorder}`}
                          />
                          <button
