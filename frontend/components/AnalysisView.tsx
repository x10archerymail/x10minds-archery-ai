...
const AnalysisView: React.FC<AnalysisViewProps> = React.memo(
  ({ type, themeMode = "dark", accentColor = "orange" }) => {
    ...
    const handleAnalyze = async () => {
      ...
      try {
        const prompt =
          type === "FORM"
            ? "Analyze these archer's form images in detail. Identify the bow type (Recurve/Compound). Check stance, grip, draw elbow position, anchor point, and alignment across all provided angles. Provide 3 specific improvements based on these images."
            : "Analyze these target face images. Estimate the total score (assuming standard 10-zone scoring). Describe the group pattern (tightness, location relative to center). Suggest potential tuning or technique adjustments based on the arrow spread shown in these photos.";
      ...
      >
        <div className="max-w-[2000px] mx-auto">
          <header className="mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
            <h2
              className={`text-3xl md:text-4xl font-black mb-3 flex items-center gap-3 ${headerText} tracking-tight`}
            >
              {type === "FORM" ? (
                <User className={`w-10 h-10 ${accentText}`} />
              ) : (
                <Target className={`w-10 h-10 ${accentText}`} />
              )}
              {type === "FORM" ? "Form Analysis" : "Target Scoring"}
            </h2>
            <p className={`${subText} text-lg max-w-2xl`}>
              {type === "FORM"
                ? "Upload up to 5 photos of your draw position from different angles for elite AI feedback."
                : "Upload clear photos of your target faces to calculate scores and analyze grouping patterns."}
            </p>
          </header>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
            ...
            <div className="space-y-6 lg:sticky lg:top-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  ...
                  className={`
                    flex flex-col items-center justify-center p-8 rounded-3xl border-2 border-dashed transition-all duration-300 group
                    ${
                      images.length >= MAX_IMAGES
                        ? "opacity-50 cursor-not-allowed border-neutral-800"
                        : isDark
                        ? "border-neutral-800 hover:border-orange-500 hover:bg-orange-500/5"
                ...
                >
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${
                      isDark
                        ? "bg-neutral-900 text-neutral-400 group-hover:text-orange-500"
                        : "bg-gray-100 text-gray-500 group-hover:text-blue-500"
                    }`}
                  >
                  ...
                  <p className={`text-xs mt-1 ${subText}`}>
                    Max 5MB each ({images.length}/{MAX_IMAGES})
                  </p>
                </button>
              ...
              {images.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 animate-in fade-in zoom-in-95 duration-500">
                  {images.map((img, idx) => (
                    <div
                      key={idx}
                      className="relative aspect-square rounded-2xl overflow-hidden group border-2 border-transparent hover:border-orange-500 transition-all shadow-xl"
                    >
                      <img
                        src={img}
                        alt={`Upload ${idx + 1}`}
              ...
              <button
                ...
                ${
                  images.length === 0
                    ...
                    : `bg-gradient-to-r ${accentGradient} text-white hover:shadow-orange-500/25`
                }
              `}
              >
                {isAnalyzing ? (
            ...
            >
              ...
              {!result && !isAnalyzing && (
                <div
                  className={`flex-1 flex flex-col items-center justify-center text-center px-6 ${subText}`}
                >
                  <div
                    className={`w-24 h-24 rounded-3xl flex items-center justify-center mb-8 rotate-3 hover:rotate-0 transition-transform duration-500 ${
                      isDark
                  ...
                  <h4 className={`font-black text-2xl mb-3 ${headerText}`}>
                    Ready for Deployment
                  </h4>
                  <p className="text-lg max-w-sm leading-relaxed">
                    Upload your photos and let X10Minds decode your performance
              ...
              {result && !isAnalyzing && (
                <div
                  className={`flex-1 overflow-y-auto custom-scrollbar pr-4 prose prose-lg prose-headings:font-black prose-p:leading-relaxed prose-li:my-1 max-w-none ${
                    isDark
                      ? "prose-invert prose-strong:text-orange-400 prose-ul:list-disc"
                      : "prose-neutral prose-strong:text-blue-600 prose-ul:list-disc"
                  }`}
                >
